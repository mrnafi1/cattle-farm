...
const EMPTY = { date: new Date().toISOString().slice(0, 10), produced: "", sold: "", pricePerLiter: "70" };

export default function DairyLog() {
  const { milkLogs, addMilkLog, updateMilkLog, deleteMilkLog } = useApp();
  const { t } = useLanguage();
  ...
  const openEdit = (log) => { setEditTarget(log); setForm({ date: log.date, produced: log.produced, sold: log.sold, pricePerLiter: log.pricePerLiter }); };
  const closeForm = () => { setShowForm(false); setEditTarget(null); setForm(EMPTY); };

  const handleSave = () => {
    const data = { date: form.date, produced: Number(form.produced), sold: Number(form.sold), pricePerLiter: Number(form.pricePerLiter) };
    if (editTarget) updateMilkLog(editTarget.id, data);
    else addMilkLog(data);
    closeForm();
  };

  const totalProduced = milkLogs.reduce((s, l) => s + l.produced, 0);
  const totalSold     = milkLogs.reduce((s, l) => s + l.sold, 0);
  const totalRevenue  = milkLogs.reduce((s, l) => s + l.sold * l.pricePerLiter, 0);
  ...
  return (
    <div className="space-y-4">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-700/50">
                {[t("date"), `${t("produced")} (L)`, `${t("sold")} (L)`, `${t("stock")} (L)`, t("pricePerLiter"), "আয়", "অ্যাকশন"].map((h) => (
      ...
      <ConfirmDialog
        ...
        onConfirm={() => { deleteMilkLog(deleteTarget.id); setDeleteTarget(null); }}
      />
    </div>
  );
}
